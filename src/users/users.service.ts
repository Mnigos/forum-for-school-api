...
export class UsersService {
  ...
  async getOneByEmail(email: string): Promise<User> {
    ...
    if (!user)
      throw new BadRequestException('Cannot find user with specify email')
  ...
  async update(user: User): Promise<User> {
    ...
    if (!updatedUser)
      throw new BadRequestException('Cannot find user with specify id')
  ...
  async delete(_id: string): Promise<boolean> {
    ...
    if (!deletedUser)
      throw new BadRequestException(
        'Something went wrong with deleting this user'
      )
